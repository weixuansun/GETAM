...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform2:
            ...
            img_dict = []
            img_dict.append(img)
            img_dict.append(ori_img)
            img_dict.append(croppings)
            img, ori_img, croppings = self.transform2(img_dict)
        img = torch.from_numpy(img)

        return name, img, ori_img, croppings


class VOC12ImageDatasetVAL(Dataset):
...
class VOC12ClsDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root, transform=None, transform2=None):
        super().__init__(img_name_list_path, voc12_root, transform, transform2)
        self.label_list = load_image_label_list_from_npy(self.img_name_list)

    def __getitem__(self, idx):
        name, img, ori_img, croppings = super().__getitem__(idx)

        label = torch.from_numpy(self.label_list[idx])

        return name, img, label, ori_img, croppings


class VOC12ClsDatasetVAL(VOC12ImageDatasetVAL):
